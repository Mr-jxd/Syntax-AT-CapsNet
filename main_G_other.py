...
def load_adj(dataset):
    ...
    train_adj, dev_adj=train_test_split(train_adj,test_size=0.1, random_state=0)
...
train=train[:8099]
train_label=train_label[:8099]
test=test[:1000]
test_label=test_label[:1000]
dev=dev[:900]
dev_label=dev_label[:900]
train_adj=train_adj[:8099]
dev_adj=dev_adj[:900]
test_adj=test_adj[:1000]
...
for epoch in range(args.num_epochs):
    ...
    for iteration in range(1, n_iterations_test + 1):
        ...
        pro=[]
        for it in probs[0]:
            pro.append(np.argmax(it))
        pro=np.array(pro)    
        preds_list = preds_list + pro.tolist()

        yl=[]
        #print('y_batch',y_batch)
        for itt in y_batch:
            yl.append(np.argmax(itt))
        yl=np.array(yl)    
       
        #print('preds_list',preds_list)
        y_list = y_list + yl.tolist()
       # print('y_list',y_list)
    ...
    [precision, recall, F1, support] = \
        precision_recall_fscore_support(y_list, preds_probs, average='macro')
